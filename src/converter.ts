...
export class Converter {
	...
	toHtml(text: string) {
		...
		// detab
		//$$$
		text = detab(text, this.opts as ConverterOptions, globals);
		...
		const boptions = this.opts as ConverterOptions;
		//run languageExtensions
		helpers.forEach(this.langExtensions, (ext: any) => {
			//$$$
			text = runExtension(text, boptions, globals, ext);
		});
		// run the sub parsers
		//$$$
		text = metadata(text, boptions, globals);
		//$$$
		text = hashPreCodeTags(text, boptions, globals);
		//$$$
		text = githubCodeBlocks(text, boptions, globals);
		//$$$
		text = hashHTMLBlocks(text, boptions, globals);
		//$$$
		text = hashCodeTags(text, boptions, globals);
		//$$$
		text = stripLinkDefinitions(text, boptions, globals);
		//$$$
		text = blockGamut(text, boptions, globals);
		text = unhashHTMLSpans(text, boptions, globals);
		text = unescapeSpecialChars(text, boptions, globals);
		...
		text = completeHTMLDocument(text, boptions, globals);
