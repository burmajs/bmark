...
export type ArgObj = {
  config: {
    delimiter: string;
    mathDelimiter: any;
    codeLangPrefix: string;
    spacesForNest: number;
    latexEnv: string[];
    subsDollar: string;
    tabTo?: string;
  };
...
export type C = {
  ...
  priority: number;
  matchRegex: RegExp;
  converter: (argBlock: any) => any | string;
  matchedString: any[];
  provisionalText?: string;
};
export type CArray = Array<C>;
//
export type ObjProperties = {
  config: ArgObj["config"];
  blockSyntax: any[];
  inlineSyntax: any[];
  addBlockSyntax: (argSyntax: any) => boolean;
  removeBlockSyntax: (argTag: any) => boolean;
  addInlineSyntax: (argSyntax: any) => boolean;
  removeInlineSyntax: (argTag: any) => boolean;
  mdInlineParserFormer: (argText: any) => any;
  mdInlineParserLatter: (argText: any) => any;
  mdInlineParser: (argText: any) => any;
  mdTBParser: (argText: any) => string;
  mdListParser: (argText: any, spacesForNest: any) => string;
  mdBlockquoteParser: (argText: any) => string;
  analyzeStructure: (argText: any) => any;
  render: (argText: any) => any;
};
//
const baganMD = (argConfig?: string[]) => {
  const mdBlockSyntax = (argObj: ArgObj) => {
  ...
  //====================================
  const mdInlineSyntax = (argObj: ArgObj) => {
  ...
  const Obj = {
    ...
    removeBlockSyntax: (argTag: string): boolean => {
      for (let ii = 0; ii < (Obj.blockSyntax || []).length; ii++)
    ...
    removeInlineSyntax: (argTag: string) => {
      for (let ii = 0; ii < (Obj.inlineSyntax || []).length; ii++)
    ...
    mdInlineParserFormer: function (argText: string) {
    ...
    mdInlineParserLatter: function (argText: string) {
    ...
    mdTBParser: (argText: string) => {
      ...
      for (let kk = 2; kk < lineText.length; kk++) {
        ...
        const colDivText = lineText[kk].replace(/\s/g, "").match(/\|+/g) as RegExpMatchArray;
        retText += "<tr>\n";
        let num = 0;
    ...
    mdListParser: (argText: string, spacesForNest: any) => {
      const checkListDepth = (argLine: string) => {
        const listType = checkListType(argLine);
        let spaceRegex: RegExp;
      ...
      const checkListType = (argLine: string) => {
      ...
      let lineDepth: number, lineType: string;
      let tempText = "";
    ...
    mdBlockquoteParser: (argText: string) => {
    ...
    analyzeStructure: (argText: string) => {
